...
/*
 * Testbench: core_m_ext_cache_tb -- core_m_ext_tb.sv's exact program,
 ...
 * core_m_ext_tb.sv (this project's own established convention). Picked
 * specifically to confirm the divider's genuine multi-cycle stall (DIV
 * holding S_EXEC for dozens of cycles, per the white-box check below)
 * composes correctly with a concurrent I$/D$ in the fetch/mem paths --
 * the divider's stall happens entirely within S_EXEC, a state neither
 * cache is ever involved in at all (their core-facing ports only ever
 * see traffic during S_FETCH/S_FETCH_HI or S_MEM/S_AMO_WRITE), so this is
 * really confirming the ABSENCE of any interaction, not a new one to get
 * right. Originally landed pointed at core_icache_harness.sv (I$ only)
 * as an earlier, narrower milestone; see core_c_ext_cache_tb.sv's own
 * header for why re-pointing here supersedes that rather than leaving a
 * near-duplicate file behind.
 *
 * Every hierarchical reference below (dut.core0.*, dut.sram0.memory[])
 * is unchanged from the original -- see core_c_ext_cache_tb.sv's own
 * header for why that's true by construction, not luck.
 */
module core_m_ext_cache_tb;
    ...
    initial begin
        ...
        $display("");
        $display("core_m_ext_cache_tb: %0d passed, %0d failed", pass_count, fail_count);
        if (fail_count > 0) $display("core_m_ext_cache_tb: FAILURES PRESENT");
        $finish;
    end
